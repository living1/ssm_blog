		...
		$.messager.confirm("系统提示","您确定要删除这<font color='red'>"+selectedRows.length+"</font>条数据吗?",function(r){
			if(r){//r表示用户点击的按钮，确认(true),取消(false)
				...
				$.post("${pageContext.request.contextPath}/admin/blogType/delete.do",{ids:ids},function(result){
					if(result.success){
						if(result.exist){//判断是否存在这个属性，如果有就返回后台封装好的判断结果
...
<table class="easyui-datagrid" id="dg" title="博客类别管理" fitColumns="true" pagination="true"
 rownumbers="true" url="${pageContext.request.contextPath }/admin/blogType/list.do" fit="true"
 toolbar="#tb">
 	<thead>
 		<tr>
 		<!-- field对应后台,这个属性的名字一定要和后台的json相对应，因为到时候会自动匹配 -->
 			...
 			<th field="typeName" width="100px" align="center">博客类型名称</th>
 		...
 		<table cellspacing="8px">
 			...
 			<tr>
 				...
 				<!-- easyui-numberbox设置这个框只能填数字 -->
 					<input type="text" id="orderNo" name="orderNo" class="easyui-numberbox" required="true" style="width:60px;"/>&nbsp;(类别序号从小到大排序)
 				</td>
 			</tr>
 		</table>
 	</form>
